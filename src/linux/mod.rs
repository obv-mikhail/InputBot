...
impl Event {
    pub fn bind<F>(self, callback: F) where F: Fn() + Send + Sync + 'static {
        ...
        if HOTKEYS.lock().unwrap().len() != 1 {return};
        spawn(move || 
            while HOTKEYS.lock().unwrap().len() != 0 {
...
fn grab_button(button: u32, display: *mut Display, window: u64) {
    unsafe {
        XGrabButton(display, button, 0, window, 0, (ButtonPressMask | ButtonReleaseMask) as u32, GrabModeAsync, GrabModeAsync, 0, 0);
    }
}
...
pub unsafe fn get_event() -> Option<Event> {
    ...
    with_display(|display| {
        ...
        for hotkey in HOTKEYS.lock().unwrap().keys() {
            match hotkey {
                &KeybdPress(key_code) | &KeybdRelease(key_code) => {
                    XGrabKey(display, key_code as i32, 0, window, 0, GrabModeAsync, GrabModeAsync);
                },
                &MousePressLeft | &MouseReleaseLeft => grab_button(Button1, display, window),
                &MousePressRight | &MouseReleaseRight => grab_button(Button3, display, window),
                &MousePressMiddle | &MouseReleaseMiddle => grab_button(Button2, display, window),
                _ => {} 
            }
        };
...
fn send_keybd_input(code: u8, is_press: i32) {
    with_display(|display| unsafe {XTestFakeKeyEvent(display, code as u32, is_press, 0);});
}
...
pub fn num_lock_is_toggled() -> bool {
    ...
    with_display(|display| unsafe {XGetKeyboardControl(display, &mut state);});
    (state.led_mask & 2 != 0)
}
...
pub fn is_pressed(code: Code) -> bool {
    ...
    with_display(|display| unsafe {XQueryKeymap(display, transmute(&mut array));});
